...
public class Board {
    ...
    public char[][] getBoardDeepCopy() {
        ...
        for (int i = 0; i < board.length; i++) boardCopy[i] = Arrays.copyOf(board[i], board[i].length);
        return boardCopy;
    }
        ...
        for (int row = 0; row < board.length; row++)
            for (int col = 0; col < board[0].length - 3; col++)
                if (board[row][col] == piece && board[row][col + 1] == piece
                        ...
                        && board[row][col + 3] == piece) return true;
        return false;
    }
        ...
        for (int row = 0; row < board.length - 3; row++)
            for (int col = 0; col < board[0].length; col++)
                if (board[row][col] == piece
                        ...
                        && board[row + 3][col] == piece) return true;
        return false;
    }
        ...
        for (int row = 3; row < board.length; row++)
            for (int col = 0; col < board[0].length - 3; col++)
                if (board[row][col] == piece
                        ...
                        && board[row - 3][col + 3] == piece) return true;
        return false;
    }
    ...
    public String toString() {
        ...
        for (char[] r : board) {
            for (char c : r)
                if (c == '\0') boardToString += " |";
                else boardToString += c + "|";
            boardToString += '\n';
        }
    ...
    public ArrayList<Integer> getPossibleMoves() {
        ...
        for (int i = 0; i < board[0].length; i++) if (isPossibleMove(i)) possibleMoves.add(i);
    ...
    private int maxDownWardDiagConsecutivePieces(char piece) {
        ...
        for (int row = 0; row < board.length - 3; row++)
            for (int col = 0; col < board[0].length - 3; col++) {
                ...
                if (board[row + 1][col + 1] == piece) count++;
                else if (count != 0 && count > maxConsecutivePieces) {
                    maxConsecutivePieces = count;
                    count = 0;
                }
                if (board[row + 2][col + 2] == piece) count++;
                else if (count != 0 && count > maxConsecutivePieces) {
                    maxConsecutivePieces = count;
                    count = 0;
                }
                if (board[row + 3][col + 3] == piece) count++;
                else if (count != 0 && count > maxConsecutivePieces) {
                    maxConsecutivePieces = count;
                    count = 0;
                }
